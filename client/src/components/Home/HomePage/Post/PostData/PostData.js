      ...
      <div className="postData__caption">
        {changeIcon ? (
          <SentimentVerySatisfiedIcon
            className={favorited ? classes.likeTrue : classes.likeNone}
        ...
        <div className="postData__caption__data">{data.caption}</div>
      </div>
      {commentsData.length === 0 ? (
        <button
          className="viewCommentBtn"
          onClick={commentViewChange}
          hidden={!hidden}
        >
          ...
          {commentsData.map((item, index) => {
            return (
              <div key={item.id} className="post__comment">
                <CommentUserInfo
                  data={commentUser[commentsData.length - (index + 1)]}
                />
                <CommentData
                  data={commentsData[commentsData.length - (index + 1)]}
                />
                <div className="comments-deleteBtn">
                  <button
                    type="submit"
                    id={commentsData[commentsData.length - (index + 1)]["id"]}
                    onClick={handleDelete}
                    hidden={
                      commentUser[commentsData.length - (index + 1)]["id"] ===
                      userId
                        ? false
                        : true
                    }
